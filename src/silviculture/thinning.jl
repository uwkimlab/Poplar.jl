@system Thinning begin
    ...
    thinning(trees, thinning_index, thinning_value) => begin
        (trees - thinning_value[thinning_index]) / u"hr"
    end ~ track(u"ha^-1/hr", when=flag_thinning)

    delN(trees, thinning) => begin
         thinning * u"hr" / trees
    ...
    thinning_WS(thinning_index, delN, trees, thinning_date, thinning_S, WS) => begin
        WS * delN * thinning_S[thinning_index] / u"hr"
    end ~ track(u"kg/ha/hr", when=flag_thinning)

    thinning_WR(thinning_index, delN, trees, thinning_date, thinning_R, WR) => begin
