@system Demand begin
    ...
    CH2O_for_protein(RPROAV, partition_foliage, protein_leaf_max, partition_stem, protein_stem_max, partition_root, protein_root_max) => begin
        RPROAV *
       (partition_foliage * protein_leaf_max +
        partition_stem * protein_stem_max +
    ...
    N_demand_veg_p(C_demand_veg_p, CH2O_for_veg_protein, partition_foliage, N_leaf_max, partition_stem, N_stem_max, partition_root, N_root_max) => begin
        (C_demand_veg_p / CH2O_for_veg_protein) * (partition_foliage * N_leaf_max + partition_stem * N_stem_max + partition_root * N_root_max)
    ...
    N_demand_old_p(N_demand_old_max) => begin
        ...
        # max(0, (WS - SDDOT - C_net_stem) * PROSTR * 0.16 - N_stem) +
    ...
    N_demand_veg(C_demand_veg, CH2O_for_veg_protein, partition_foliage, N_leaf_max, partition_stem, N_stem_max, partition_root, N_root_max) => begin
        (C_demand_veg / CH2O_for_veg_protein) * (partition_foliage*N_leaf_max + partition_stem * N_stem_max + partition_root * N_root_max#= + FRSTR * FNINSR=#)
    end ~ track(u"g/m^2/hr")
